...
public class Credit_total extends JFrame {
	...
	public Credit_total() {
		...
		select = new JButton("선택하기");
		select.setFont(new Font("휴면고딕체", Font.BOLD,17));
		c.add(select);
		select.setBounds(420, 550, 120, 30);
		
		select.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(jc.getSelectedItem().toString() == a[1]) {
					setVisible(false);
					Credit2 frame = new Credit2();
				}
				else if(jc.getSelectedItem().toString() == a[2])
					{
						setVisible(false);
...
public static void main(String[] args) {
		Credit_total frame = new Credit_total();
	}
}
